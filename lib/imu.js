  ...
  LSM303D: {
    ...
    REGISTER: {
      value: {
        ...
        /* the temperature is 0x05 - 0x06 in the following order, this sensor is not enabled by default
          t lsb
          t msb
         */
        TEMP: 0x05,
        ...
        /*
        the magnetometer registers are 0x08 - 0x0D in the order below
         x lsb
         x msb
         y lsb
         y msb
        ...
        /*
          Accelerometer registers are (0x28 - 0x2D) in the order below
          x lsb
          x msb
          y lsb
          y msb
    ...
    initialize: {
      value: function (board, opts){
        ...
        // based on Pololu drivers etc.
        // PO. https://www.pololu.com/file/download/LSM303D.pdf?file_id=0J703
        // Accelerometer
        ...
        // PO. p.36 8.18
        // 7. ABW
        // 6. ABW - Acclerometer AA filter. see PO. table 39
        // 5. AFS
        // 4. AFS
        // 3. AFS - Acceleration Full Scale selection, see PO. table 40.
        // 2. 0
        // 1. self test mode 0 = off, 1 = enabled
        // 0. SPI mode 0 = 4 wire, 1 = 3 wirte
       io.i2cWrite(address, this.REGISTER.CTRL2, 0x00);
        ...
        // 6. AODR
        // 5. AODR
        // 4. AODR - Accelerometer data rate selection see PO. table 36
        // 3. BDU - Block data update - 0 = continuous, 1 = after MSB/LSB read
        // 2. AZEN - Z axis enable 0 = off, 1 = enabled
        // 1. AYEN - Y axis enable 0 = off, 1 = enabled
        // 0. AXEN - X axis enable 0 = off, 1 = enabled
        io.i2cWrite(address, this.REGISTER.CTRL1, 0x57);
        ...
        //
        // PO. p.39 8.21
        // 7. TEMP_EN enable thermometer 0 = off, 1 = on
        // 6. M_RES
        // 5. M_RES Magnometer resolution - 00 = Low Resolution 11 = High resolution
        // 4. M_ODR
        // 3. M_ODR
        // 2. M_ODR - Magnometer Data rate - see PO. table 47
        // 1. LIR1 - Latch interrupt request on INT2_SRC register - 0 = no latch, 1 = latch
        // 0. LIR2 - Latch interrupt request on INT1_SRC register
        io.i2cWrite(address, this.REGISTER.CTRL5, 0x64);
        ...
        // 7. 0
        // 6. MFS
        // 5. MFS - Magnetic full-scale selection, see PO. table 50
        // 4. 0
        // 3. 0
        // 2. 0
        // 1. 0
        // 0. 0
        io.i2cWrite(address, this.REGISTER.CTRL6, 0x20);
        ...
        // PO. p.40 8.23
        // 7. AHPM
        // 6. AHPM - Accelerometer high pass filter mode, seet PO. table 50
        // 5. AFDS - Filtered acceleration data selection, 0 bypass filter
        // 4. T_ONLY
        // 3. 0 Temperature sensor only, 1 = temp enabled when when
        // 2. MLP - Low Power Mode
        // 1. MD
        // 0. MD
        io.i2cWrite(address, this.REGISTER.CTRL7, 0x00);

        //set up the read for the magnetometer - note msb assertion for sub address reads
        io.i2cRead(address, this.REGISTER.MAG | (1 << 7), 6, function (data) {
          computed.magnetometer = {
            x: int16(data[1], data[0]),
            y: int16(data[3], data[2]),
            z: int16(data[5], data[4])
        ...
        //set up the read for the accelerometer - note msb assertion for sub address reads
        io.i2cRead(address, this.REGISTER.ACCEL | (1 << 7), 6, function (data) {
          computed.accelerometer = {
            x: int16(data[1], data[0]),
            y: int16(data[3], data[2]),
            z: int16(data[5], data[4])
    ...
    identifier: {
      value: function(opts) {
        var address = opts.address || Drivers["LSM303D"].ADDRESSES.value[0];
        return "lsm303d-" + address;
      }
    }
  },
  LSM6: {
    ...
    initialize: {
      value: function (board, opts) {
        ...
        // based on Pololu drivers etc.
        // PO. https://www.pololu.com/file/0J1087/LSM6DS33.pdf
        // Accelerometer
        ...
        io.i2cWrite(address, this.REGISTER.CTRL1_XL, 0x80);

        // Gyro

        // 0x80 = 0b010000000
        // ODR = 1000 (1.66 kHz (high performance)); FS_XL = 00 (245 dps)
        io.i2cWrite(address, this.REGISTER.CTRL2_G, 0x80);

        // Common

        // 0x04 = 0b00000100
        // IF_INC = 1 (automatically increment register address)
        io.i2cWrite(address, this.REGISTER.CTRL3_C, 0x04);

        //set up the read for the accelerometer/gyro - don't need to assert for multiple writes
        io.i2cRead(address, this.REGISTER.OUTX_L_G, 12, function (data) {
          computed.gyro = {
            x: int16(data[1], data[0]),
            y: int16(data[3], data[2]),
            z: int16(data[5], data[4])
    ...
    identifier: {
      value: function(opts) {
        var address = opts.address || Drivers["LSM6"].ADDRESSES.value[0];
        return "lsm6-" + address;
      }
    }
  },
...
var Controllers = {
  ...
  LSM303D: {
    ...
    components: {
      value: ["accelerometer", "magnetometer"]
    },
    accelerometer: {
      get: function() {
        return priv.get(this).accelerometer;
  ...
  //the Pololu minimu9 - v3 (https://www.pololu.com/product/2468) is a carrier board
  // with a LSM303D and a L3GD20H on it,
  // this is a convenience helper to combine the two chips into one controller
  // the adafruit 10dof - https://www.adafruit.com/products/1604 and 9-DOF
  // https://www.adafruit.com/products/1714
  // also have the same chips, so *should* also work (both Adafruit and Pololu have boards
  // that support the individual sensors)
  // the default addresses used are for the Pololu board as I don't have
  // the Adafruit part. It does not wrap in the barometer part on the AltIMU or the 10 DOD Adafruit boards
  // as they use different barometer parts.
  MINIMU9V3: {
    initialize: {
      value: function (opts) {
        ...
        var imuOpts = Object.assign({
          freq: opts.freq,
          board: this.board,
        }, opts);

        //now that we are here, we need to unset this
        imuOpts.controller = "LSM303D";
        imuOpts.address = opts.addressLSM303D || null;

        state.imu = new IMU(imuOpts);

        var gyroOpts = Object.assign({
          freq: opts.freq,
          board: this.board,
        }, opts);

        //now that we are here, we need to unset this
        gyroOpts.controller = "L3GD20H";
        gyroOpts.address = opts.addressL3GD20H || null;


        state.gyro = new Gyro(gyroOpts);

      }
    },
    components: {
      value: ["accelerometer", "magnetometer","gyro"]
    },
    accelerometer: {
      get: function() {
        return priv.get(this).imu.accelerometer;
      }
    },
    magnetometer: {
      get: function() {
        return priv.get(this).imu.magnetometer;
      }
    },
    gyro: {
      get: function() {
...
Controllers["ALTIMUv4"] = Controllers.MINIMU9V3;

//these are boards by Adafruit that have the same sensors on them as the MinIMU-9/AltIMU-10
Controllers["AF9DOFIMU"] = Controllers.MINIMU9V3;
Controllers["AF10DOFIMU"] = Controllers.MINIMU9V3;
