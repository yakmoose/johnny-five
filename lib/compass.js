...
var Controllers = {
  ...
  LSM303D: {
    initialize: {
      value: function(opts, dataHandler) {
        var IMU = require("./imu");
        var driver = IMU.Drivers.get(this.board, "LSM303D", opts);
        var state = priv.get(this);

        // PO. p.10 Magnetometer sensitivity
        // magnetometer returns in mgauss (4g default, will need to factor in this if we want to cusomise the sensitivity)
        // so turn to gauss, can't see any sort of conventions about internal units.
        // possibly needs to be a discussion.
        state.sensitivity = 0.160 / 1000;
    ...
    toScaledHeading: {
      value: function (raw) {
        //this is a triple axis chip, need to find a better algo than just x/y
        // or we move this to the imu or orientaiton for proper tilt compensation and
