...
var Controllers = {
  ...
  BNO055: {
    ...
    toGravity: {
      value: function(raw) {
        var state = priv.get(this);
        return raw / state.sensitivity;//the BNO currently defaults to acceleration, not G..
      }
    }
  },
  ...
  LSM303D: {
    initialize: {
      value: function(opts, dataHandler) {
        ...
        //PO. p.10, 0,061 converts to mg -> / 1000 to get in g
        state.sensitivity =  0.061 / 1000;
  ...
  LSM6: {
    initialize: {
      value: function(opts, dataHandler) {
        var IMU = require("./imu");
        var driver = IMU.Drivers.get(this.board, "LSM6", opts);
        var state = priv.get(this);

        //PO. p.15, 0.061 converts to mg -> / 1000 to get in g
        state.sensitivity =  0.061 / 1000;
